!*******************************************************************
!  The model is designed to simulate the boundary layer response to *
!  a presure field of a tropical cyclones in gradient gradient wind *
!  balance at the top of the boundary layer with a multi-level      *
!  hydrostatic, elastic model on an f-plane or on a beta-plan       *
!  The model was designed and coded by Yuqing Wang                  *
...
program PBL_MODEL
  ...
  !  Model variables as usually used
  ...
  !   jt0, it0, x- and y-grid indice of TC center at initiall time
  !   ds - grid spacing
  !   dt - time step
  !   kfi - total integration time in hours
  !   khr - Output units, 1 means every hour
  !   index - 0-f-plane, 1-beta-plane
  !   nst - whether the periodic zonal boundary condition is used
       ! 1 - cyclic zonal boundary condition is used
       ! 0 - no cyclic zonal boundary condition is used
  !   fnm - output fine name prefix
  !   pro_wind - wind profile, H10 and WILLOUBY for now
  ...
  ! read time series of TC info. from ebtrk every 10 minutes
  ...
  ! JAMES test for N. Hemisphere storms
   ! lat_1d=lat_1d*-1.
   ! Print*, lat_1d
  ! JAMES test for N. Hemisphere storms
  ...
  !-----------------------------------
  ! to read lat-lon, landuse and terrain data
  !-----------------------------------
...
29   format(27x, f4.0)
  ...
  ! if rmax is missing,calculates Rmax using Eq. 7a from Willoughby et al. (2006):
  ! fix spd_gl
  do k = 1, n_bdy
  ...
  ! inhr: time steps in an hour
  ! hr_o: time steps for 10-minute output hr_o
  inhr = int(3600.0/dt + 0.1)
  hr_o = int(60*10/dt + 0.1)

  !--------------------------------
  ! to calculate Coriolis parameter
  !--------------------------------
  ! f = 2*omega*sin(phi)
  ...
  !----------------------------------------------------------
  ! to specify the lateral boundary layer buffer zone width
  !----------------------------------------------------------
  do kk=1,nbd
    bnd(kk)=0.125d0*real(nbd-kk)/real(nbd-1)
  enddo
  !-----------------------------------------------------------
  ! lateral boundary damping coefficient in the buffer zone
  !-----------------------------------------------------------
  do kk=1,nbd
  ...
  !-----------------------------------------
  !  to calculate the initial moving speed of the TC
  !-----------------------------------------
  nhr   = 0
  ...
  !-----------------------------------------------
  !  to initialise the cyclone centre at each mesh
  !-----------------------------------------------
  txp =txp_1d(1)
  typ =typ_1d(1)
  vmt =vm_1d(1)
  ...
  !------------------------------------------------------------
  ! to initialize the model wind & pressure gradient fields
  !-------------------------------------------------------------
  akv=0.0
  w=0.0
  ...
  if(mod(nt-1,inhr).eq.0) then
    nhr=(nt-1)/inhr
  !+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  !  to determine the cyclone parameters such as center, maximum wind.
  !-------------------------------------------------------------------
    txpc=txp_1d(nmh)
    typc=typ_1d(nmh)
 ...
 121   format(1x,'hr=',i3,', txp=',f8.3,', typ=',f8.3,', vm=',f5.1, &
  ...
  !+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  !  to write the results during the time integration such as K.E
  !-------------------------------------------------------------------
  ...
  !
   ...
   ! write out after 10 minute
   if(mod(nt-1, hr_o).eq.0) then

     ! call cal_slp(slp, lq, lp, ds, prx, pry, txp, typ, pct)
  ...
  !
    ...
    write(21) ((slp(j,i),j=1,nx),i=1,ny)
     ...
     ! write(21) ((prx(j,i),j=1,nx),i=1,ny)

     ! write(21) ((pry(j,i),j=1,nx),i=1,ny)

    close(21)
  endif

  if(nhr.lt.kfi) then
    ! wind update 10min at 4,9,14,19
...
!
  subroutine inner(u,v,w,f_2d,prx,pry,akv,IL,JL,ds,dt,top,land, &
    ...
    ! inner performs explicit time integration
    ! computes time tendencies for all variables:
    ! scheme employed for time-differencing is due to
    ! m. miller (qjrms,1978) or matsuno (via option)
  !
   ut=0.0
   vt=0.0
...
!$omp parallel do default(shared)
  !$omp& private(i,j,k,uu,vv,vadu,vadv,coru,corv,ww,wwa,wwb)
    do i=1,IL
     do k=1,km
      do j=1,JL
       uu=u(i,j,k)
       vv=v(i,j,k)
  !    vertical advection first-order upwind scheme
...
!$omp end parallel do
  ...
  !
  !    for even time-steps complete a matsuno time integration
  ...
  do i=1,IL
   do k=1,km
    do j=1,JL
     uu=up(i,j,k)
     vv=vp(i,j,k)
  !   vertical advection first-order upwind sckeme
     ww=0.5*(w(i,j,k)+w(i,j,k+1))
     wwa=0.5*(ww-abs(ww))/dzu(k)
     wwb=0.5*(ww+abs(ww))/dzu(k-1)
     if(k.eq.1) then
...
!
!----------------------------------------------------------
!  To calculate horizontal advection tendency of u, v winds
!----------------------------------------------------------
...
!$omp parallel do default(shared)
  !$omp& private(i,j,k,advx,im1,im2,im3,ip1,ip2,ip3)
    do j=1,JL
     do k=1,km
       do i=1,IL
       ip1=i+1
...
subroutine hdiff(ut,vt,u,v,akh,bw,lq,lp,km,ds,dt,nnest)
  ...
  !-------------------------------------------------------------
  !  To include linear 4th-order horizontal diffusion term using
  !  the t-1 time level fields to ensure the numerical stability
  !-------------------------------------------------------------
  !
  if(nnest.eq.1) then
  !
!$omp parallel do default(shared)
  !$omp& private(j,i,k,jm1,jp1,dm1,dm2,dm)
  do i=2,lp-1
  do k=1,km
  do j=1,lq
  jp1=j+1
...
subroutine vdiff(ut,vt,u,v,ak,IL,JL,km,dt,zz,dzz,dzu,top,land,cd)
    ...
    enddo
!$omp parallel do default(shared)
  !$omp& private(i,j,k,spd,pu,pv,akm,sh,ax,bx,cx,xa,xb,xc,z,al,al2)
    do i=1,IL
      do k=2,km
      do j=1,JL
      ! z=        top(i,j)+zz(k)*(1.0-top(i,j)/hh)
      z=min(hh, top(i,j)+zz(k)*(1.0-min(top(i,j)/hh,0.90)))
      al=0.4*z/(1.0+0.4*z/al0)
      al2=al*al
  !    &       (dzu(k-1)*(1.0-top(i,j)/zz(km+1)))**2
...
subroutine tridiag(ax,bx,cx,yy,lq,km)
  !
  !++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  !  To solve a tri-diagonal matrix using a direct solver
  !--------------------------------------------------------------
  implicit none
...
subroutine www(w,u,v,top,lq,lp,km,ds,zz,zu,dzz,nnest)
  ...
  !
!$omp parallel do default(shared)
  !$omp& private(j,i,k,jm1,jp1,w1,w2,w3,term,fc,fc2)
  do i=2,lp-1
  do j=1,lq
  jm1=j-1
  jp1=j+1
  ...
  ! Ming
  ! w3=-term*(1.0-zu(k)/hh)/(1.0-top(j,i)/hh)
...
subroutine bound(t,lq,lp,bnd,nbd,km,nnest)
  !
  !++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  !  to damp the boundary noise for prognostic variable t using special
  !  smoothing operation with open lateral boundary conditions
  !--------------------------------------------------------------------
  implicit none
...
subroutine bound1(p,lq,lp,bnd,nbd)
  !
  !++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  !  to damp the boundary noise for p using a special smoothing
  !  operation with open boundary conditions
  !----------------------------------------------------------------
  implicit none
...
subroutine bound1c(p,lq,lp,bnd,nbd)
 ...
 subroutine H10(u,v,f_2d,IL,JL,km,ds,vm,rm,txc,tyc,r34,pc,pe, &
   ...
   ! pe = environment pressure value is set to climatology
   ! ee = base of natural logarithms
   ! sfc density = 1.13 (at 980mb)
   ! eqn 7 in Holland 2010.
   ! Assume Vmax and Pmin are reliablly observed
   ! It is not critical and a constant value of density can e used.
   !  pe = 1010.
   ! fac is a reduction factor from gradient winf to 10-m wind
   ! Ming Ge: March 2018. adjust Vmax by removing forward motion of storm
  ...
  ! JAMES v2
  ! Change the reduction factor to 0.76
    ! fac = 0.85
  ! Change reduction factor to 0.71
   ! fac = 0.76
   fac = 0.71
  ! JAMES v2

  !  calculate xx_it in eq. 10  at R34, v=17
  !  when rr = r34, xxterm = xx_it = 1
  xx_it = 0.5
...
!$omp parallel do default(shared)
  !$omp& private(j,i,k,tx,ty,rr,term1,xxterm,vt,corr_fac)
  do j=1,JL
     ty = (real(j)-tyc)*ds
     do i=1,IL
       tx = (real(i)-txc)*ds
  ...
  ! JAMES v2
  ! Decided to add the assymetry in the post-processing
       corr_fac = 0.
        ! corr_fac = 2.0*(rm*rr)/(rm**2 + rr**2)
  ! +                 *1.173*spd_tc**0.63/spd_tc
  ! JAMES v2


  ! JAMES v2
  ! add a correction to get the correct sign of the U and V winds in each hemisphere
  ...
  ! JAMES v2
  ! remove the special treatment of the lowest four levels.
        ! do k=1,4
        !   u(i,j,k)=-vt*ty*(fac+(k-1)*(1.-fac)/4.0)
  ! +                 + corr_fac*ux
  !         v(i,j,k)= vt*tx*(fac+(k-1)*(1.-fac)/4.0)
  ! +                 + corr_fac*vy
  !       end do
  ! JAMES v2
...
!
 subroutine H10_slp(slp,IL,JL,ds,vm,rm,txc,tyc,pc,pe)
   ...
   ! Using eqn 2 in Holland 2010:
   ! slp = pc + (pe - pc)*exp(-1.*(rm/r)**b)
   ! ee = base of natural logarithms
   ! sfc density = 1.13 (at 980mb)
   ee = 2.718281
   vm_sfc_sym = vm
   ...
   ! fac is a reduction factor from gradient wind to 10-m wind
   fac = 0.71

   ! calculate b from bs from Holland et al. (2010)
   b = bs*(1/(fac*fac))

  !c      find TC center and set pc
    ! ic = int(txc)
    ! jc = int(tyc)
    ! slp(ic, jc) = pc
...
!$omp parallel do default(shared)
  !$omp& private(j,i,tx,ty,rr,term1,slp_tmp)
  do j=1,JL
     ty = (real(j)-tyc)*ds
     do i=1,IL
       tx = (real(i)-txc)*ds
...
subroutine prxy_H10(u, v, prx,pry,f_2d,IL,JL,km, ds,vm,rm,r34, &
  ...
  ! Change the reduction factor to 0.76
    ! fac = 0.85
  ! Change the reduction factor to 0.71
    ! fac = 0.76
   fac = 0.71
  ! JAMES v2
...
!  to get estimated symmetric surface-level tangential winds from gradient-levels winds,
!  uses a method from Knaff et al. (2011).
!  This method uses a reduction factor of 0.90 up to a radius(rr0) of 100 km
!  a reduction factor of 0.75 for any radius 700 km or greater
!  and a linear decreasing reduction factor for any radius between those two radius values
!  If the point is over land (true for any county), this reduction factor is further reduced by 20%
...
! under construction
! calculate gradient wind from symmetric surface wind
...
! Calculate the surface wind direction from gradient winds
! the function adds an inflow angle to the gradient wind direction.
! because surface friction changes the wind direction
! The inflow angle is calculated as a function of the distance from the storm center
! to the location and the storm's Rmax at that observation point (eq. 11a--c, Phadke et al. 2003):
...
! add back in wind from forward speed of TC
subroutine  wind_sfc(u_sfc,v_sfc,spd_sfc_sym,dir_sfc,rmax,rr_dist &
        ,tcspd_u, tcspd_v)
  implicit none
  real(kind=4) :: u_sfc,v_sfc
...
! calculate all the inputs needed for the Willoughby wind profile
! equation 10 a-c
! lat_tc: Latitude, in decimal degrees
subroutine get_wind_param_eq10(X1, NN, AA, vm_gl, lat_tc)

  ! JAMES v2
  ! Convert latitude to absolute values of latitude so the parameters are the same in each hemisphere
  ...
  ! JAMES v2
   ...
   return
end subroutine get_wind_param_eq10

! calculate all the inputs needed for the Willoughby wind profile
! equation 11 a-c
! lat_tc: Latitude, in decimal degrees
! rm: radius of maximum wind.
subroutine get_wind_param_eq11(X1, NN, AA, vm_gl, lat_tc, rm)
  ...
  ! JAMES v2
   ...
   return
end subroutine get_wind_param_eq11

! calculate lower(R1) and upper(R2) boundary of the transition zone (km)
! assumes that  R2 − R1 (the width of the transition region) is 25 kilometers
! when Rmax is larger than 20 kilometers and 15 kilometers otherwise.
...
! Polynomial ramp function
! when x, the nondimensional argument, is  <= zero, wgt=0
! when x >= 1 wgt =1,
! First and second derivitives are zero at both ends, C2 continuity.
real function wgt(x)
...
! Uses interval bisection to find coordinate XX of a given
! weighting WW on the polynomial ramp function
! Returns: Nondimensional coordinate XX=[(r-R1)/(R2-R1)] where
! the polynomial ramp function, wgt, is equal to WW
! This subroutine is adapted from Hugh Willoughby
...
real function correction_vt(vt_tc) result(val)
  ...
  ! calculates Rmax using Eq. 7a from Willoughby et al. (2006):
...
subroutine willouby(u,v,f_2d,IL,JL,km,ds,vm,rm,txc,tyc,ux,vy,lat &
  ...
  ! JAMES v2
  ! Changed the reduction factor to 0.76
   ! reduction_factor = 0.85
  ! Changed the reduction factor to 0.71
   ! reduction_factor = 0.76
  ...
  !
!$omp parallel do default(shared)
  !$omp& private(j,i,k,tx,ty,rr,vt, fac, correction_factor)
  ...
  ! JAMES v2
  ! Remove the double counting of the reduction factor 'fac'.
  ! The reduction factor was applied earlier in this subroutine.
       ! vt = VofR2(rr,vm_gl, rm, R1,R2, WID, NN, X1, X2, AA)/fac
      vt = VofR2(rr,vm_gl, rm, R1,R2, WID, NN, X1, X2, AA)
  ! JAMES v2


  ! JAMES v2
  ! Decided to add the asymmetry i the post-processing
      correction_factor = 0.0
       ! correction_factor = 2.0*(rm*rr)/(rm**2 + rr**2)
  ! +                        *1.173*spd_tc**0.63/spd_tc
  ! JAMES v2

  ! JAMES v2
  ! Added a correction to get the correct sign of the U and V winds in each hemisphere
  ...
  ! JAMES v2
  ! Remove this different treatment of the lowest levels.
        ! do k=1,4
        !   u(i,j,k)=-vt*ty*(fac+(k-1)*(1.-fac)/4.0)
  ! +                 /rr + correction_factor*ux
  !         v(i,j,k)= vt*tx*(fac+(k-1)*(1.-fac)/4.0)
  ! +                 /rr + correction_factor*vy
  !       end do
  ! JAMES v2
    end do
  end do
...
subroutine prxy_willouby(prx,pry,f_2d,IL,JL,km,ds,vm,rm,txc,tyc, &
  ...
  ! Changed the reduction factor to 0.71
   ! reduction_factor = 0.76
  reduction_factor = 0.71
  ! Justified in KW01 showed it ranges from .9 to .7 around the storm,
  ! and that is the factor from gradient to 22 m.
  ! We are going to 10 m so it'll be even more justified at the .7 level.
  ! Vickerey shows a factor of about 0.72 going from the supergradient jet to sfc.
  ! We don't allow enough time for jet to form so we artifically put it in.
  ! In anycase, 0.76 is well within 1sd of the mean (0.81) of 150 dropsode obs of Powell.
  ! JAMES v2
...
!$omp parallel do default(shared)
  ...
  ! JAMES v2
  ! Decided to add the asymmetry in the post-processing
      correction_factor = 0.0
       ! correction_factor = 2.0*(rm*rr)/(rm**2 + rr**2)
  ! +                       *1.173*spd_tc**0.63/spd_tc
  ! JAMES v2
...
! --------------------------------------------
! Sectionally continuous wind-profile function with a
! power law inside the eye and a double-length exponential
! decay outside. There is a smooth polynimial transition
! across the radius of maximum wind.
! Returns: Double exponent wind profile as a function of radius
! Adapted from Hugh Willoughby
real function VofR2(RR,Vmax,Rmax,R1,R2,WID,ENI,XL1,XL2,AA)
  ...
  ! !In transition, Compute weighted combination of inside and outside profiles
    ...
    VofR2 =(1.0-w)*          Vin(RR,Vmax,Rmax,ENI) &
          + w*((1.0-AA)*Vout(RR,Vmax,Rmax,XL1) &
          + AA *Vout(RR,Vmax,Rmax,XL2))
...
! --------------------------------------------
! Sectionally continuous Willoughby wind-profile function with a
! power law inside the eye and a hyperbolic cosecant
! decay outside. There is a smooth polynimial transition
! across the radius of maximum wind.
! Returns: Swirling wind as a function or radius
...
! --------------------------------------------
! Power-law wind profile for inside the eye
...
! --------------------------------------------
! Single-length exponential for outisde the eye
...
! --------------------------------------------
! Hyperbolic cosecant outer profile
