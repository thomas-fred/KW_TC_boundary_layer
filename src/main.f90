...
!$omp parallel do default(shared) private(i,j,k,advx)
      do j=1,JL
         do k=1,km
...
!$omp parallel do default(shared) private(i,j,k,advy)
   do i=1,IL
      do k=1,km
...
!
   if(nnest.eq.1) then
!
!$omp parallel do default(shared) private(j,i,k,jm1,jp1,dm1,dm2,dm)
      do 10 i=2,lp-1
         do 10 k=1,km
            do 10 j=1,lq
               jp1=j+1
...
!$omp parallel do default(shared) private(j,i,k,jm1,jp1)
      do 20 i=2,lp-1
         do 20 k=1,km
            do 20 j=1,lq
               jp1=j+1
...
!
!$omp parallel do default(shared) private(j,i,k,dm1,dm2,dm)
      do 30 i=2,lp-1
         do 30 k=1,km
            do 30 j=2,lq-1
...
!
!$omp parallel do default(shared) private(j,i,k,to)
   do 9 k=1,km
!
      do 4 i=1,lp
         do 4 j=1,lq
...
!$omp parallel do default(shared) private(j,i,tx,ty,rr,term1,slp_tmp)
   do j=1,JL
      ty = (real(j)-tyc)*ds
      do i=1,IL
         tx = (real(i)-txc)*ds
